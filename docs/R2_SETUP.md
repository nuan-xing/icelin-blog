...
3. 在图片字段中拖拽或选择图片，也可以点“浏览图库”直接复用 R2 中已有图片。图库默认显示当前文件夹，并可切换全部 R2、已添加、未添加和搜索；浏览器会把新图片交给 Worker，Worker 再写入 R2，浏览器不持有 R2 密钥。
...
“R2 图库”实时列出 bucket 对象，并根据 D1 中的图片引用显示“已添加”或“未添加”。图片选择器也会复用这套状态：其中“未添加”表示图片已经在 R2、但尚未被内容引用；本地尚未上传的文件仍需通过选择或拖拽上传。被引用的图片默认不能删除；如确有需要，可在编辑室中确认强制删除。
